"""
    ...
    def _resolve_validator_key() -> str:
      """
      Return effective validator key for the scheduler.
      Prefers VIT_VALIDATOR_KEY, falls back to GENESIS_TREASURY_KEY,
      then auto-generates a deterministic key on testnet.
      On mainnet, raises ValueError if no key is set.
      """
      key = settings.VIT_VALIDATOR_KEY.strip() or settings.GENESIS_TREASURY_KEY.strip()
      if not key:
          ...
          logger.warning(
              "[scheduler] VIT_VALIDATOR_KEY not set — using auto-generated deterministic key "
